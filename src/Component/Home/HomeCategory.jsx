    ...
    <div className='py-5 p-1 category_bgimg'>
      <div className='container mb-5'>
        ...
        <ul className="nav justify-content-center nav-brand-tabs mt-3 flex-wrap">
          {productlist.map((item, index) => (
            <li className="nav-item" key={index}>
              <button
                className={`nav-link shadow m-2 rounded-pill bg-warning text-dark ${
                  activeBrand.brandId === item.brandId ? 'active' : ''
                }`}
        ...
        >
          ...
          <div className='ms-2 '>
             <Link to={"/product"} className='text-decoration-none'> <ButtonCom btn={"Explore Now"} /></Link>
          </div>
          </div>
        </div>
      </div>
